...
The privileged vs unprivileged distinction is about mount capability, not verification strategy. Mounting EROFS requires `CAP_SYS_ADMIN` in the init user namespace; unprivileged users fall back to FUSE-based mounting via `composefs-fuse`. However, the FUSE daemon uses the same repository object access path as the kernel EROFS mount — it calls `repo.open_object()` to open content objects from the repository's object store.

Object-level integrity depends on the repository's filesystem, not on the mount mechanism. If the repository lives on a filesystem with fsverity enabled, the kernel verifies every object at `open_object()` time — the same verification that occurs on the kernel EROFS path. If the repository is in insecure mode (no fsverity on the underlying filesystem), objects are served without integrity verification, again the same as the kernel path. There is no separate userspace digest verification fallback in the FUSE daemon.

Signature verification (PKCS#7) remains relevant at the image/mount level, independent of the mount mechanism. Before mounting, composefs tooling or policy engines can verify that the composefs digest was vouched for by a trusted signer, using signature artifacts attached to the OCI image. This check is the same whether mounting via EROFS or FUSE.

The trust anchor for signature verification is the set of trusted certificates. Several options exist for storing these.
...
**Linux user keyring.** The `@u` (user) keyring is managed per-UID and accessible without privileges. Composefs tooling could read trusted certificates from the user keyring via `keyctl`. This integrates with Linux's existing key management infrastructure but adds a `keyutils` dependency.
