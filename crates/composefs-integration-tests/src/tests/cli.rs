...
fn test_v1_repo_uses_v1_by_default() -> Result<()> {
    ...
    // Init a V1 repo using the init subcommand's --erofs-version flag
    let repo_dir = tempfile::tempdir()?;
    let repo = repo_dir.path();
    cmd!(
        sh,
    ...
    // Verify meta.json records erofs_version: 1
    let meta_json = std::fs::read_to_string(repo.join("meta.json"))?;
    assert!(
        meta_json.contains("\"erofs_version\": 1") || meta_json.contains("\"erofs_version\":1"),
        "meta.json should contain erofs_version: 1, got: {meta_json}"
    );
