...
pub async fn fetch_and_import_referrers<ObjectID: FsVerityHashValue>(
    ...
    for entry in &index.manifests {
        ...
        let manifest: oci_spec::image::ImageManifest =
            serde_json::from_slice(&raw_manifest_bytes).with_context(|| {
        ...
        let config_verity = if let Some(v) = repo.has_stream(&config_content_id)? {
            ...
            let mut config_stream =
                repo.create_stream(crate::skopeo::OCI_CONFIG_CONTENT_TYPE);
        ...
        for layer_desc in manifest.layers() {
            ...
            let layer_verity = if let Some(v) = repo.has_stream(&layer_content_id)? {
                ...
                let mut layer_stream =
                    repo.create_stream(crate::skopeo::OCI_BLOB_CONTENT_TYPE);
        ...
        let mut manifest_stream =
            repo.create_stream(crate::skopeo::OCI_MANIFEST_CONTENT_TYPE);
...
async fn fetch_referrers_by_tag(
    ...
    let (raw_bytes, _digest) = client
        ...
        .await
        .with_context(|| {
            format!("fetching referrers via tag scheme (tag '{tag}')")
        })?;
