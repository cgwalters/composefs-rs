...
pub async fn run_app(args: App) -> Result<()> {
    ...
    {
        // Prefer the subcommand-level --erofs-version; fall back to global flag; then default V2.
    ...
    match args.cmd {
        Command::ComputeId { fs_opts } => {
            ...
            let id = composefs::fsverity::compute_verity::<ObjectID>(
                &composefs::erofs::writer::mkfs_erofs_versioned(&fs, version),
            );
            println!("{}", id.to_hex());
        }
    ...
    match args.cmd {
        ...
        Command::Oci { cmd: oci_cmd } => match oci_cmd {
            ...
            OciCommand::ComputeId { config_opts } => {
                let fs = load_filesystem_from_oci_image(&repo, config_opts)?;
                let id = fs.compute_image_id(&repo);
                println!("{}", id.to_hex());
            }
            OciCommand::Pull {
                ref image,
