...
use composefs::{
    ...
    erofs::writer::{ValidatedFileSystem, mkfs_erofs_v1_min_version},
    fs::{
...
pub(crate) fn run() -> Result<()> {
    ...
    // min_composefs_version mirrors C's `options->version` start value.
    let min_composefs_version = args.min_version;
    ...
    let image = mkfs_erofs_v1_min_version(&ValidatedFileSystem::new(fs)?, min_composefs_version);
...
fn apply_transformations(fs: &mut FileSystem<Sha256HashValue>, args: &Args) -> Result<()> {
    ...
    // Always add overlay whiteout stubs (we always produce V1/C-compatible format).
