...
//! format versions, inline/external files, special file types, xattrs, directory
...
fn main() {
    ...
    // 2. Empty root — V1_0 (compact inodes, whiteout table)
    {
        let mut fs = empty_root();
        fs.add_overlay_whiteouts();
        let image = mkfs_erofs(&fs, FormatVersion::V1_0);
        seeds.push(("empty_root_v1_0", image.into()));
    }

    // 3. Single inline file (small content stored in inode)
    {
        let mut fs = empty_root();
        insert_leaf(
            &mut fs.root,
    ...
    // 4. Single external (chunk-based) regular file
    {
        let mut fs = empty_root();
    ...
    // 10. Nested directories: /a/b/c/file
    {
        let mut fs = empty_root();
    ...
    // 11. Many entries (20+ files to exercise multi-block directories)
    {
        let mut fs = empty_root();
        for i in 0..25 {
    ...
    // 13. Mixed types — one of every file type in a single directory
    {
        let mut fs = empty_root();
        insert_leaf(
            &mut fs.root,
    ...
    // 14. Hardlink — two entries sharing the same Rc<Leaf> (nlink > 1)
    {
        let mut fs = empty_root();
    ...
    // 15. Large inline — file with maximum inline content (just under 4096 bytes)
    {
        let mut fs = empty_root();
    ...
    // 16. Deep nesting — 8 levels of directories
    {
        let mut fs = empty_root();
    ...
    // 17. Nonzero mtime — exercises V1_0 build_time min calculation
    {
        ...
        // Also generate a V1_0 variant which uses build_time = min(mtimes)
        let image_v11 = mkfs_erofs(&fs, FormatVersion::V1_1);
        seeds.push(("nonzero_mtime", image_v11.into()));

        fs.add_overlay_whiteouts();
        let image_v10 = mkfs_erofs(&fs, FormatVersion::V1_0);
        seeds.push(("nonzero_mtime_v1_0", image_v10.into()));
    }

    // 18. Large uid/gid — forces extended inodes even in V1_0
    {
        ...
        // V1_0 with large uid/gid: forces extended inodes
        fs.add_overlay_whiteouts();
        let image = mkfs_erofs(&fs, FormatVersion::V1_0);
...
fn makedev(major: u32, minor: u32) -> u64 {
    // Linux makedev: ((major & 0xfffff000) << 32) | ((major & 0xfff) << 8)
