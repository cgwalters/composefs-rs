...
impl<'a, ObjectID: FsVerityHashValue> InodeCollector<'a, ObjectID> {
    ...
    fn collect_leaf(&mut self, leaf_id: LeafId) -> InodeIdx {
        ...
        if matches!(leaf.content, tree::LeafContent::CharacterDevice(0)) && nlink > 1 {
            panic!("invalid composefs filesystem: whiteout inode has nlink > 1");
        }
...
fn prepare_erofs_inodes<'a, ObjectID: FsVerityHashValue>(
    ...
    let (header_flags, composefs_version) = if version == format::FormatVersion::V1 {
        ...
        let cfs_ver = if has_user_whiteout { 1u32 } else { 0u32 };
...
fn mkfs_erofs_inner<ObjectID: FsVerityHashValue>(
...
pub(crate) fn mkfs_erofs_with_faults<ObjectID: FsVerityHashValue>(
    ...
    mkfs_erofs_inner(fs, version, Some(faults))
}
