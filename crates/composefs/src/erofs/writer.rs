...
//! - [`mkfs_erofs_v1_min_version`] — like `mkfs_erofs_versioned` for V1, but with explicit
//!   control over the minimum `composefs_version` field (mirrors `mkcomposefs --min-version`)
//!
//! All three require a [`ValidatedFileSystem`], which is the type-safe gate: constructing
...
//! ## Format versions
//!
//! Two on-disk formats are supported, selected by [`FormatVersion`]:
//!
//! **V1** (`FormatVersion::V1`) is byte-for-byte compatible with the C `mkcomposefs` tool.
//! It uses compact inodes (32 bytes) where the inode fits, extended (64 bytes) otherwise;
//! collects inodes in BFS order; includes a 256-entry whiteout stub table at the start of
//! the inode area; sets `build_time` to the minimum mtime; and encodes user-visible whiteout
//! files (chr 0,0) via `trusted.overlay.opaque=x` xattrs rather than storing them directly.
//! The `composefs_version` header field is 0 normally and auto-upgrades to 1 when user
//! whiteouts are present.
//!
//! **V2** (`FormatVersion::V2`, the default) is the composefs-rs native format. It always
//! uses extended inodes (64 bytes), collects inodes in DFS order, omits the whiteout stub
//! table, sets `build_time` to 0, and sets `composefs_version` to 2. Whiteout files are
//! stored without escaping.
//!
...
impl InodeXAttrs {
    ...
    fn add(&mut self, name: &[u8], value: &[u8], version: format::FormatVersion) {
        for (idx, prefix) in format::XATTR_PREFIXES.iter().enumerate().rev() {
            ...
            if version == format::FormatVersion::V1 && idx == 5 {
                continue;
            }
    ...
    fn write_inode(&self, output: &mut impl Output, idx: usize, ctx: &mut WriteContext) {
        ...
        if use_compact {
            ...
            let ino = match version {
                ...
                format::FormatVersion::V2 => (output.len() / INODE_SLOT_SIZE) as u32,
            };

            // V2 does not store sub-second mtime precision (mtime_nsec=0),
            // matching origin/main which used ..Default::default() to zero it.
            // V1 preserves full nanosecond precision.
            let mtime_nsec: u32 = match version {
                format::FormatVersion::V1 => self.stat.st_mtim_nsec,
...
impl<'a, ObjectID: FsVerityHashValue> InodeCollector<'a, ObjectID> {
    ...
    fn collect_tree(&mut self, root: &'a tree::Directory<ObjectID>) {
        ...
        while let Some((dir, parent, me)) = queue.pop_front() {
                ...
                match inode {
                    ...
                    tree::Inode::Leaf(leaf_id, _) => {
                        ...
                        if is_canonical
                            ...
                            && self.version == format::FormatVersion::V1
                            && !is_stub
            ...
            if self.version == format::FormatVersion::V1 && dir_has_whiteout {
                self.inodes[me]
                    .xattrs
...
fn share_xattrs(
    ...
    let (xattrs, shared): (BTreeMap<XAttr, usize>, Vec<XAttr>) = match version {
        format::FormatVersion::V1 => {
...
fn write_erofs(
    ...
    let xattr_blkaddr = match version {
        format::FormatVersion::V1 => output.get_xattr_blkaddr(),
        format::FormatVersion::V2 => 0,
    };
...
///
/// Collects inodes from the filesystem, injects the V1 opaque xattr on the
...
///
/// `min_composefs_version` is only meaningful for V1: it sets a lower bound on
/// the `composefs_version` header field, mirroring C mkcomposefs `--min-version`.
/// Pass `0` for the default auto-upgrade behaviour (0→1 when user whiteouts present).
/// Pass `1` to force `composefs_version=1` even when no user whiteouts exist.
///
...
fn prepare_erofs_inodes<'a, ObjectID: FsVerityHashValue>(
    ...
    // For V1, compute header flags and composefs_version matching C mkcomposefs behavior.
    // This must be checked before share_xattrs(), while all xattrs are still local.
    let (header_flags, composefs_version) = if version == format::FormatVersion::V1 {
        ...
        // C mkcomposefs bumps composefs_version from 0 to 1 when any user-provided
        // node is a whiteout (char device with rdev=0).  In the Rust writer, such
        // nodes are detected and marked as `escaped_whiteout` by InodeCollector
        // (stubs added by add_overlay_whiteouts() are deliberately excluded from
        // this flag via the root-level 2-hex-char name heuristic).
        // `min_composefs_version` mirrors C's `--min-version=1`: it forces
        // composefs_version=1 even when no user whiteouts are present.
        let has_user_whiteout = inodes.iter().any(|inode| inode.escaped_whiteout);
        let cfs_ver = if has_user_whiteout || min_composefs_version >= 1 {
            1u32
        } else {
            0u32
        };
        ...
        (0u32, format::COMPOSEFS_VERSION.get())
    };
...
pub(crate) fn mkfs_erofs_inner<ObjectID: FsVerityHashValue>(
    ...
    let (inodes, xattrs, min_mtime, header_flags, composefs_version) =
        prepare_erofs_inodes(fs, version, min_composefs_version);
...
/// - `FormatVersion::V1`: C mkcomposefs compatible (compact inodes, BFS). The 256
///   overlay whiteout stubs (`00`–`ff`) required by V1 are added automatically.
...
/// Like [`mkfs_erofs_versioned`] for V1, but with an explicit minimum
/// `composefs_version` header value.
///
/// This mirrors C mkcomposefs `--min-version=N`: passing `min_composefs_version=1`
/// forces `composefs_version=1` in the EROFS header even when no user-visible
/// whiteout devices are present (the default auto-upgrade only sets it to 1
/// when a whiteout is encountered).
///
/// `min_composefs_version` must be 0 or 1; values above 1 are clamped to 1
/// (there is no C-compatible format above version 1).
pub fn mkfs_erofs_v1_min_version<ObjectID: FsVerityHashValue>(
    fs: &mut ValidatedFileSystem<ObjectID>,
    min_composefs_version: u32,
) -> Box<[u8]> {
    mkfs_erofs_inner(
...
pub(crate) fn mkfs_erofs_with_faults<ObjectID: FsVerityHashValue>(
    ...
    mkfs_erofs_inner(&mut fs.0, version, 0, Some(faults))
}
