...
impl<ObjectID: FsVerityHashValue> Inode<'_, ObjectID> {
    ...
    fn pad_inline_tail_v2(&self, output: &mut impl Output, inode_and_xattr_size: u64, size: u64) {
        ...
        let inline_start = inline_start
            .checked_add(inode_and_xattr_size)
            .expect("image position + inode header size cannot overflow u64");
        // Does the inline tail (block_offset(size) bytes starting at inline_start)
        // overflow the block it starts in?  If so, pad inline_start to the next
        // block boundary so the tail lands cleanly in a fresh block.
        if block_offset(inline_start) + block_offset(size) > block_size {
            if let Some(pad_size) = bytes_to_block_boundary(inline_start) {
                output.write_zeros(pad_size as usize);
