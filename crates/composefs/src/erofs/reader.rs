...
impl<'img> Image<'img> {
    ...
    pub fn restrict_to_composefs(mut self) -> Result<Self, ErofsReaderError> {
        ...
        if cv != COMPOSEFS_VERSION.get() && cv != COMPOSEFS_VERSION_V1.get() && cv != 1 {
            return Err(ErofsReaderError::InvalidImage(format!(
...
"#;
        ...
        let image = mkfs_erofs(&ValidatedFileSystem::new(fs).unwrap());
...
"#;
    ...
    fn round_trip_dumpfile(input: &str) -> (String, String) {
        ...
        let image = mkfs_erofs(&ValidatedFileSystem::new(fs_orig).unwrap());
...
"#;
        ...
        let vfs_orig = ValidatedFileSystem::new(fs_orig).unwrap();
        let image = mkfs_erofs(&vfs_orig);
        let fs_rt = erofs_to_filesystem::<Sha256HashValue>(&image).unwrap();
...
"#;
    ...
    fn test_restrict_to_composefs_rejects_unsupported_features() {
        ...
        let dumpfile = "/ 0 40755 2 0 0 0 1000.0 - - -\n";
        let fs = dumpfile_to_filesystem::<Sha256HashValue>(dumpfile).unwrap();
        let base_image = mkfs_erofs(&ValidatedFileSystem::new(fs).unwrap());
...
"#;
    ...
    fn test_inode_blocks_rejects_oversized_range() {
        ...
        let fs = dumpfile_to_filesystem::<Sha256HashValue>(dumpfile).unwrap();
        let mut image = mkfs_erofs(&ValidatedFileSystem::new(fs).unwrap());
    ...
    mod proptest_tests {
        ...
        fn round_trip_filesystem<ObjectID: FsVerityHashValue>(spec: FsSpec) {
            ...
            let image = mkfs_erofs(&ValidatedFileSystem::new(fs_write).unwrap());
        ...
        fn round_trip_filesystem_v1<ObjectID: FsVerityHashValue>(spec: FsSpec) {
            ...
            let image = mkfs_erofs_versioned(
                &ValidatedFileSystem::new(fs_write).unwrap(),
                FormatVersion::V1,
            );
        ...
        fn test_c_composefs_info_reads_v1() {
            ...
            runner
                .run(&filesystem_spec(), |spec| {
                    ...
                    let image = mkfs_erofs_versioned(
                        &ValidatedFileSystem::new(fs).unwrap(),
                        FormatVersion::V1,
                    );
        ...
        /// tree (no whiteout stubs) and adds the 256 stubs internally, while the Rust
        /// writer operates on the in-memory tree after `add_overlay_whiteouts()`.
        ///
        ...
        fn test_v1_binary_identical_to_c_mkcomposefs() {
            ...
            runner
                .run(&filesystem_spec(), |spec| {
                    ...
                    //   fs_c  — user entries only, serialized as a dumpfile and fed to
                    //           C mkcomposefs (which adds the 256 whiteout stubs internally)
                    //   fs_rs — user entries + stubs added by add_overlay_whiteouts(), fed
                    //           directly to our Rust V1 writer
                    //
                    // This mirrors the production flow: C receives a dumpfile without
                    // the stubs and adds them itself, while Rust adds them in-process.
                    // Using the same spec for both ensures the user-level content matches.
                    let fs_c = build_filesystem::<Sha256HashValue>(spec.clone());
                    let mut fs_rs = build_filesystem::<Sha256HashValue>(spec);
                    fs_rs.add_overlay_whiteouts();
                    ...
                    // Get our Rust V1 writer binary output (stubs already in fs_rs)
                    let rust_image = mkfs_erofs_versioned(
                        &ValidatedFileSystem::new(fs_rs).unwrap(),
                        FormatVersion::V1,
                    );
        ...
        fn test_v1_binary_identical_unusual_content() {
            ...
            runner
                .run(&unusual_filesystem_spec(), |spec| {
                    ...
                    let mut fs_rs = build_unusual_filesystem::<Sha256HashValue>(spec);
                    fs_rs.add_overlay_whiteouts();
        ...
        fn test_v1_binary_diff_diagnostic() {
            ...
            //   - fs_rs has add_overlay_whiteouts() called on it, fed to Rust writer
            let dumpfile = "\
/ 0 40000 3 0 0 0 0.0 - - -\n\
...
/m.A3Q_rRtSZ20o_ 0 @120000 - - - - 0.0 /B/u - -\n";
            ...
            // Build fs_rs (with stubs) for our Rust writer.
            // C receives the dumpfile directly (without stubs) and adds them internally.
            let mut fs_rs = dumpfile_to_filesystem::<Sha256HashValue>(dumpfile).unwrap();
            fs_rs.add_overlay_whiteouts();

            let c_image = c_mkcomposefs_from_dumpfile(dumpfile);
            let rust_image =
                mkfs_erofs_versioned(&ValidatedFileSystem::new(fs_rs).unwrap(), FormatVersion::V1);
...
"#;
    ...
    fn test_v1_symlink_block_boundary_euclean_regression() {
        ...
        let fs = dumpfile_to_filesystem::<Sha256HashValue>(&dumpfile).unwrap();
        let image = mkfs_erofs_versioned(&ValidatedFileSystem::new(fs).unwrap(), FormatVersion::V1);
    ...
    fn test_v1_inline_layout_validator_catches_bad_layout() {
        ...
        let vfs = ValidatedFileSystem::new(fs).unwrap();

        // The good image must pass validation.
        let good_image = mkfs_erofs_versioned(&vfs, FormatVersion::V1);
        ...
        let bad_image = mkfs_erofs_with_faults(&vfs, FormatVersion::V1, faults);
    ...
    fn test_negative_mtime_does_not_corrupt_build_time() {
        ...
        // add_overlay_whiteouts is required for V1 compatibility
        fs.add_overlay_whiteouts();
