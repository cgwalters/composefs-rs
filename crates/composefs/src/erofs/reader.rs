        ...
        fn test_v1_binary_identical_to_c_mkcomposefs() {
            ...
            runner
                .run(&filesystem_spec(), |spec| {
                    ...
                    let rust_image = mkfs_erofs_versioned(&fs_rs, FormatVersion::V1);
...
"#;
    ...
    fn test_v1_inline_layout_validator_catches_bad_layout() {
        ...
        let good_image = mkfs_erofs_versioned(&fs, FormatVersion::V1);
        ...
        let bad_image = mkfs_erofs_with_faults(&fs, FormatVersion::V1, faults);
    ...
    /// Hardlinked whiteouts (chardev rdev=0 with nlink > 1) must be rejected by both
    /// the writer and the reader. A whiteout represents the absence of a file in an
    /// overlay; nlink > 1 is semantically meaningless and should be an error.
    #[test]
    fn test_hardlinked_whiteout_rejected() {
        ...
        // The writer must reject a hardlinked whiteout.
        ...
        assert!(
            result.is_err(),
            "writer should panic on hardlinked whiteout"
        );
    }
