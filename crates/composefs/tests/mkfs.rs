...
use composefs::{
    ...
    erofs::{
        ...
        writer::{
            ValidatedFileSystem, mkfs_erofs, mkfs_erofs_v1_min_version, mkfs_erofs_versioned,
        },
    },
...
fn test_fsck() {
    foreach_case(|fs| {
        ...
        tmp.write_all(&mkfs_erofs(&ValidatedFileSystem::new(fs).unwrap()))
            .unwrap();
...
fn test_vs_mkcomposefs() {
    for case in [empty, simple] {
        ...
        let image = mkfs_erofs_versioned(
            &ValidatedFileSystem::new(fs_rust).unwrap(),
            FormatVersion::V1,
        );
...
fn test_vs_mkcomposefs_min_version_1() {
    ...
    let mut fs_rust = dumpfile_to_filesystem::<Sha256HashValue>(dump).unwrap();
    fs_rust.add_overlay_whiteouts();
    let rust_image = mkfs_erofs_v1_min_version(&ValidatedFileSystem::new(fs_rust).unwrap(), 1);
