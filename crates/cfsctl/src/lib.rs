...
///
/// Uses the canonical path definitions from [`Repository`] to avoid
/// duplicating path constants.
fn resolve_repo_path(args: &App) -> Result<PathBuf> {
    if let Some(path) = &args.repo {
        Ok(path.clone())
    } else if args.system {
        Ok(Repository::<Sha256HashValue>::system_path())
    } else if args.user {
        Repository::<Sha256HashValue>::user_path()
...
fn run_init(algorithm: &Algorithm, path: Option<&Path>, insecure: bool, args: &App) -> Result<()> {
    ...
    // When not insecure, enable fs-verity on meta.json to signal that
    // the repository requires verity on all objects.
