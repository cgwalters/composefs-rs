...
integration_test!(test_cstor_vs_skopeo_equivalence);
...
fn test_cstor_idempotent_import() -> Result<()> {
    let rt = tokio::runtime::Runtime::new()?;
    rt.block_on(async {
        println!("Building test image...");
        let test_image = build_test_image()?;
        ...
        // Check import stats: second import should show layers already present
        let first_stats = first_result.stats.as_ref().expect("cstor import should have stats");
        let second_stats = second_result.stats.as_ref().expect("cstor import should have stats");
        println!("First import stats: {:?}", first_stats);
        println!("Second import stats: {:?}", second_stats);

        // The second import should have all layers already present
        assert_eq!(
            second_stats.layers, second_stats.layers_already_present,
            "second import should find all layers already present"
        );
        ...
        println!(
            "  Second import: {}/{} layers from cache",
            second_stats.layers_already_present, second_stats.layers
        );
...
integration_test!(test_cstor_idempotent_import);
...
fn test_cstor_import_with_reference() -> Result<()> {
    let rt = tokio::runtime::Runtime::new()?;
    rt.block_on(async {
        println!("Building test image...");
        let test_image = build_test_image()?;
...
integration_test!(test_cstor_import_with_reference);
